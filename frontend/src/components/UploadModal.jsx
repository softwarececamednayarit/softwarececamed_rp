...
const SectionHeader = ({ color, title }) => (
  <h3 className={`text-[10px] font-black ${color} uppercase tracking-[0.2em] flex items-center gap-2`}>
    <span>{title}</span>
  </h3>
);
...
const UploadModal = ({ isOpen, onClose, archivoParaEditar = null }) => {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" translate="no">
          <motion.div 
            initial={{ opacity: 0 }} 
            animate={{ opacity: 1 }} 
            exit={{ opacity: 0 }}
                        ...
                        {file ? (
                          <div className="space-y-3">
                            ...
                            <div className="flex flex-wrap justify-center gap-2 mt-3">
                               <Badge color="bg-emerald-100 text-emerald-700"><Zap size={10} fill="currentColor" /> <span>{stats.percent}% más ligero</span></Badge>
                               <button type="button" onClick={() => setFile(null)} className="text-[10px] font-bold text-slate-400 hover:text-rose-500 transition-colors flex items-center gap-1">
                                 <RotateCcw size={12}/> <span>Cambiar archivo</span>
                               </button>
                            </div>
                          </div>
                        ) : ( <p className="text-sm font-bold text-slate-600"><span>Haz clic o arrastra el Oficio (PDF)</span></p> )}
                      </div>
                    </div>
                  ) : (
                    <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 ml-1"><span>Nombre del expediente en Drive</span></p>
