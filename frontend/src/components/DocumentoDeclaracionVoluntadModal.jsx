...
import { X, Save, FileText, FileCheck, User, Calendar, Stethoscope, MapPin, Phone, CreditCard, Briefcase, Eye } from 'lucide-react';
import toast from 'react-hot-toast';
import { AtendidosService } from '../services/atendidosService';
import { generarPDFDeclaracionVoluntad } from '../utils/pdfGenerator'; 

const DocumentoDeclaracionVoluntadModal = ({ isOpen, onClose, item }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    representanteMed_nombre: existingData.representanteMed_nombre || '',
    representanteMed_domicilio: existingData.representanteMed_domicilio || '',
    representanteMed_colonia: existingData.representanteMed_colonia || '',
    representanteMed_ciudad: existingData.representanteMed_ciudad || 'Tepic, Nayarit',
    representanteMed_telefono: existingData.representanteMed_telefono || '',
    representanteMed_cedula: existingData.representanteMed_cedula || ''
  });
  ...
  const handleGeneratePDF = async () => {
    setGenerando(true);
    const toastId = toast.loading('Guardando y generando declaración...');
    ...
    try {
      ...
      generarPDFDeclaracionVoluntad(expActualizado);
  ...
  const handlePreviewPDF = async () => {
    ...
    try {
      ...
      generarPDFDeclaracionVoluntad(expActualizado, 'previsualizar');
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[95vh] animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-emerald-50/50 rounded-t-2xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-black text-slate-800">Declaración de Voluntad</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <h3 className="text-sm font-black text-emerald-900 uppercase tracking-wider flex items-center gap-2 border-b border-emerald-100 pb-2">
                <Stethoscope size={16} /> Médico y Quejoso
              </h3>
              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1">Nombre del Médico (Aceptante)</label>
              ...
              <div>
                ...
                <input type="text" name="nombre_usuario" value={formData.nombre_usuario} onChange={handleChangeForm} className="w-full p-2.5 bg-white border border-slate-200 rounded-lg text-sm font-medium focus:ring-2 focus:ring-emerald-500 outline-none"/>
              </div>
            </div>

            <div className="space-y-4">
              <h3 className="text-sm font-black text-emerald-900 uppercase tracking-wider flex items-center gap-2 border-b border-emerald-100 pb-2">
                <Calendar size={16} /> Fecha de Aceptación (Impresión)
              </h3>
              <div>
          ...
          <div className="space-y-4 bg-slate-50 p-5 rounded-xl border border-slate-100">
            <h3 className="text-sm font-black text-slate-700 uppercase tracking-wider flex items-center gap-2 border-b border-slate-200 pb-2">
              <Briefcase size={16} /> Datos del Representante Legal del Prestador
            </h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                <label className="block text-xs font-bold text-slate-600 mb-1">Nombre Completo</label>
                <input type="text" name="representanteMed_nombre" value={formData.representanteMed_nombre} onChange={handleChangeForm} className="w-full p-2.5 bg-white border border-slate-200 rounded-lg text-sm font-medium focus:ring-2 focus:ring-emerald-500 outline-none"/>
              </div>
              ...
              <div>
                ...
                <input type="text" name="representanteMed_cedula" value={formData.representanteMed_cedula} onChange={handleChangeForm} className="w-full p-2.5 bg-white border border-slate-200 rounded-lg text-sm font-medium focus:ring-2 focus:ring-emerald-500 outline-none"/>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-600 mb-1 flex items-center gap-1"><Phone size={14}/> Teléfono</label>
                <input type="text" name="representanteMed_telefono" value={formData.representanteMed_telefono} onChange={handleChangeForm} className="w-full p-2.5 bg-white border border-slate-200 rounded-lg text-sm font-medium focus:ring-2 focus:ring-emerald-500 outline-none"/>
              </div>
              ...
              <div className="md:col-span-2 space-y-3 mt-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="md:col-span-1">
                    ...
                    <input type="text" name="representanteMed_domicilio" value={formData.representanteMed_domicilio} onChange={handleChangeForm} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none"/>
                  </div>
                  <div className="md:col-span-1">
                    <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Colonia</label>
                    <input type="text" name="representanteMed_colonia" value={formData.representanteMed_colonia} onChange={handleChangeForm} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none"/>
                  </div>
                  <div className="md:col-span-1">
                    <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Ciudad y Estado</label>
                    <input type="text" name="representanteMed_ciudad" value={formData.representanteMed_ciudad} onChange={handleChangeForm} className="w-full p-2 bg-white border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-emerald-500 outline-none"/>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between rounded-b-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <FileText size={16} />
              {generando ? 'Generando...' : 'Generar PDF Voluntad'}
            </button>
          </div>
        </div>
...
export default DocumentoDeclaracionVoluntadModal;